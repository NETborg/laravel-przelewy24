...
class RandGenerator {
	...
	public static function generate($qty=10, $type=5)
	{
		...
		if (is_string($type)){
			if (in_array($type,array("small-letters", "s-l", "sl"))) $type = 1;
			else if (in_array($type,array("capital-letters", "c-l", "cl"))) $type = 2;
			else if (in_array($type,array("numbers","n"))) $type = 3;
			else if (in_array($type,array("letters", "all-letters", "just-letters", "only-letters", "l"))) $type = 4;
			else $type = 5;
		}
	...
	public static function strToHex($string) {
		$s='';
		foreach (str_split($string) as $c) $s.=sprintf("%02X",ord($c));
		return($s);
	}
